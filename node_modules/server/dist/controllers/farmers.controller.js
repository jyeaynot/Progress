...
function buildListFilters(search, barangay) {
    const clauses = [];
    const values = [];
    if (search && search.trim().length > 0) {
        values.push(`%${search.trim()}%`);
        const searchIndex = values.length;
        clauses.push(`(CONCAT_WS(' ', f.first_name, f.middle_name, f.last_name) ILIKE $${searchIndex}
...
export async function getFarmers(req, res) {
    try {
        ...
        const pageSize = Math.max(1, Number(typeof req.query.pageSize === "string" ? req.query.pageSize : DEFAULT_PAGE_SIZE) ||
            DEFAULT_PAGE_SIZE);
...
export async function exportFarmers(req, res) {
    try {
        const search = typeof req.query.search === "string" ? req.query.search : "";
        const barangay = typeof req.query.barangay === "string" ? req.query.barangay : "";
        const { clauses, values } = buildListFilters(search, barangay);
...
export async function createFarmer(req, res) {
    try {
        const { rsbsaId, firstName, middleName, lastName, barangay, contactNumber, civilStatus, ethnicity, birthDate, cropType, season, latitude, longitude, } = req.body;
...
export async function createInputAllocation(req, res) {
    try {
